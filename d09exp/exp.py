    ...
    def play(self):
        ...
        if stimulus_filename:
            ...
            if audio_backend == 'kivy':
                ...
                audio_pygame.mixer.music.load(stimulus_filename)
                while audio_pygame.mixer.music.get_busy():
                    continue
            else:
                self.exit_on_error("Midi Error", "Unrecognized MIDI backend")
        else:
            self.exit_on_error("Midi Error", "No Midi File Available")
